...
func TestAssetReportBindingAvailableFailsClosedOnDrift(t *testing.T) {
	...
	allowed, err := handler.assetReportBindingAvailable(context.Background(), &config, "form", &itemTypeID, &workspaceID)
	if err != nil || !allowed {
		t.Fatalf("valid binding = (%v, %v), want (true, nil)", allowed, err)
	}

	otherWorkspaceID := workspaceID + 100
	allowed, err = handler.assetReportBindingAvailable(context.Background(), &config, "form", &itemTypeID, &otherWorkspaceID)
	if err != nil || allowed {
		t.Fatalf("unserved workspace binding = (%v, %v), want (false, nil)", allowed, err)
	}
	missingItemTypeID := itemTypeID + 100
	allowed, err = handler.assetReportBindingAvailable(context.Background(), &config, "form", &missingItemTypeID, nil)
	if err != nil || allowed {
		t.Fatalf("missing item type binding = (%v, %v), want (false, nil)", allowed, err)
	}
	allowed, err = handler.assetReportBindingAvailable(context.Background(), &config, "direct", nil, &otherWorkspaceID)
	if err != nil || !allowed {
...
func TestLoadAssetReportFieldsOnlyReturnsFieldsBoundToCreateScreen(t *testing.T) {
	...
	fields, err := (&PortalHandler{db: db}).loadAssetReportFields(context.Background(), reportID)
	if err != nil {
