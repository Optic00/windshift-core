...
func TestListSchedulesIncludesCompleteOverviewForTeamMembers(t *testing.T) {
	...
	if len(memberSchedules) != 1 || len(memberSchedules[0].Layers) != 1 || len(memberSchedules[0].Layers[0].Members) != 1 {
		t.Fatalf("member schedules = %+v, want complete schedule graph", memberSchedules)
	}
	current := memberSchedules[0].CurrentOnCall
	if current == nil || len(current.OnCall) != 1 || current.OnCall[0].UserID != memberID || current.OnCall[0].UserName != "Team Member" {
		t.Fatalf("current on-call = %+v, want team member %d", current, memberID)
	}
	...
	if len(viewerSchedules) != 1 || viewerSchedules[0].CurrentOnCall != nil {
		t.Fatalf("viewer schedules = %+v, want metadata without protected current roster", viewerSchedules)
	}
}
