...
func setupOrgTicketsHandler(t *testing.T, userID int) (*PortalCustomersHandler, func(orgID int) *httptest.ResponseRecorder) {
	...
	if _, err := db.Exec(`INSERT INTO workspaces (id, name, key, active, is_personal) VALUES (10, 'Locked WS', 'LOCK', 1, 0)`); err != nil {
	...
	if _, err := db.Exec(`
		INSERT INTO items (workspace_id, workspace_item_number, item_type_id, title, description, creator_portal_customer_id)
		SELECT 10, 1, (SELECT id FROM item_types LIMIT 1), 'Org ticket #1', '', 500
	`); err != nil {
