...
func TestStartFIDOLoginDoesNotEnumerateAccountOrCredentialState(t *testing.T) {
	...
	config, err := wswebauthn.NewConfig(
		"localhost", "Windshift Test", []string{"http://localhost"}, true, "", "", false, false,
	)
	if err != nil {
		t.Fatalf("NewConfig: %v", err)
	}
	sessionManager := auth.NewSessionManager(db, false, false, nil, "webauthn-test-secret")
