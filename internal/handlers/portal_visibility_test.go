...
func TestVisibilityJSONFailsClosedWhenMalformed(t *testing.T) {
	...
	assetReport := &models.AssetReport{ID: 8}
	if err := applyAssetReportVisibility(assetReport, malformed, sql.NullString{}); err == nil {
		t.Fatal("asset report accepted malformed visibility JSON")
	}
}
