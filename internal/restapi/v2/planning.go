...
func registerPlanningRoutes(builder *routeBuilder, planning planningApplication) {
	builder.Page("/milestones", AuthAuthenticated, []string{"items:read"}, listMilestones(planning))
	builder.JSON(http.MethodPost, "/milestones", http.StatusCreated, false, AuthAuthenticated, []string{"items:write"}, createMilestone(planning))
	builder.Read("/milestones/{milestone_id}", AuthAuthenticated, []string{"items:read"}, getMilestone(planning))
	builder.JSON(http.MethodPatch, "/milestones/{milestone_id}", http.StatusOK, true, AuthAuthenticated, []string{"items:write"}, patchMilestone(planning))
	builder.Command(http.MethodDelete, "/milestones/{milestone_id}", AuthAuthenticated, []string{"items:delete"}, deleteMilestone(planning))
	builder.JSON(http.MethodPost, "/milestones/{milestone_id}/release", http.StatusOK, false, AuthAuthenticated, []string{"items:write"}, releaseMilestone(planning))
	builder.Read("/milestones/{milestone_id}/progress", AuthAuthenticated, []string{"items:read"}, milestoneProgress(planning))
	builder.Read("/milestones/{milestone_id}/test-statistics", AuthAuthenticated, []string{"items:read"}, milestoneTestStatistics(planning))
	builder.Read("/milestones/test-statistics", AuthAuthenticated, []string{"items:read"}, milestoneTestStatisticsBatch(planning))
	builder.JSON(http.MethodPost, "/milestones/reorder", http.StatusOK, false, AuthAuthenticated, []string{"items:write"}, reorderMilestones(planning))

	builder.Page("/iterations", AuthAuthenticated, []string{"items:read"}, listIterations(planning))
	builder.JSON(http.MethodPost, "/iterations", http.StatusCreated, false, AuthAuthenticated, []string{"items:write"}, createIteration(planning))
	builder.Read("/iterations/{iteration_id}", AuthAuthenticated, []string{"items:read"}, getIteration(planning))
	builder.JSON(http.MethodPatch, "/iterations/{iteration_id}", http.StatusOK, true, AuthAuthenticated, []string{"items:write"}, patchIteration(planning))
	builder.Command(http.MethodDelete, "/iterations/{iteration_id}", AuthAuthenticated, []string{"items:delete"}, deleteIteration(planning))
	builder.JSON(http.MethodPost, "/iterations/{iteration_id}/complete", http.StatusOK, false, AuthAuthenticated, []string{"items:write"}, completeIteration(planning))
	builder.Read("/iterations/{iteration_id}/progress", AuthAuthenticated, []string{"items:read"}, iterationProgress(planning))
	builder.Read("/iterations/{iteration_id}/burndown", AuthAuthenticated, []string{"items:read"}, iterationBurndown(planning))
	builder.Read("/iterations/progress", AuthAuthenticated, []string{"items:read"}, iterationProgressBatch(planning))
}
