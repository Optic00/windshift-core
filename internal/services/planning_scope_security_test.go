...
func seedPlanningScopeFixture(t *testing.T) planningScopeFixture {
	...
	`)
	itemA := planningScopeInsertID(t, db, `
		INSERT INTO items (workspace_id, workspace_item_number, title, description, status_id, iteration_id, last_active_at)
		VALUES (?, 1, 'Visible item', '', ?, ?, CURRENT_TIMESTAMP)
	`, workspaceA, statusID, iterationID)
	itemB := planningScopeInsertID(t, db, `
		INSERT INTO items (workspace_id, workspace_item_number, title, description, status_id, iteration_id, last_active_at)
		VALUES (?, 1, 'Hidden item', '', ?, ?, CURRENT_TIMESTAMP)
	`, workspaceB, statusID, iterationID)
