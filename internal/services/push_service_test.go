...
func TestPushServiceBoundsQueueWithoutGoroutinePerNotification(t *testing.T) {
	...
	if !service.Enqueue(models.Notification{ID: 1, UserID: 1, Title: "one"}) {
		t.Fatal("first push rejected")
	}
	<-started
	if !service.Enqueue(models.Notification{ID: 2, UserID: 1, Title: "two"}) ||
		!service.Enqueue(models.Notification{ID: 3, UserID: 1, Title: "three"}) {
		t.Fatal("queue rejected within capacity")
	}
	if service.Enqueue(models.Notification{ID: 4, UserID: 1, Title: "four"}) {
...
func TestPushServiceRetriesAndCapsSubscriptionFanout(t *testing.T) {
	...
	service := newPushService(db, enabledPushConfig(), serviceConfig, sender)
	if !service.Enqueue(models.Notification{ID: 1, UserID: 1, Title: "fanout"}) {
...
func TestPushServiceSerializesSameUserAndBoundsGlobalWorkers(t *testing.T) {
	...
	service := newPushService(db, enabledPushConfig(), serviceConfig, sender)
	service.Enqueue(models.Notification{ID: 1, UserID: 1, Title: "one"})
	service.Enqueue(models.Notification{ID: 2, UserID: 2, Title: "two"})
	service.Enqueue(models.Notification{ID: 3, UserID: 1, Title: "three"})
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	if err := service.Close(ctx); err != nil {
...
func TestPushServiceShutdownDeadlineReportsUndrainedWork(t *testing.T) {
	...
	service := newPushService(db, enabledPushConfig(), serviceConfig, sender)
	service.Enqueue(models.Notification{ID: 1, UserID: 1, Title: "blocked"})
	<-started
