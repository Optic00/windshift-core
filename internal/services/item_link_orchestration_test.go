...
func TestItemLinkService_EndpointVisible_ItemUsesWorkspaceKey(t *testing.T) {
	...
	// Empty workspaceKey on an item endpoint should pass (legacy callers
	// that don't pre-join the key).
	if !s.EndpointVisible("item", 0, "", accessibleKeys, nil, nil) {
		t.Fatal("empty workspace key on item endpoint should be visible (legacy)")
	}
}
