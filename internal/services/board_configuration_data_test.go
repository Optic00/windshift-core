...
func TestItemCQLWorkspaceProjectionDoesNotLoadMatchingItems(t *testing.T) {
	...
	for _, item := range []struct {
		...
		{workspaceOne, 1, "Board match one"},
		{workspaceOne, 2, "Unrelated"},
		{workspaceTwo, 1, "Board match two"},
		{workspaceDenied, 1, "Board denied"},
	} {
		if _, err := db.ExecWrite(
			`INSERT INTO items (workspace_id, workspace_item_number, title) VALUES (?, ?, ?)`,
			item.workspaceID, item.number, item.title,
		); err != nil {
			t.Fatalf("insert item %q: %v", item.title, err)
		}
	}
