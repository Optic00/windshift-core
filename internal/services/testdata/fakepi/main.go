// Fakepi is a deterministic stand-in for pi-coding-agent in RPC mode that
// the PiRunner unit tests shell out to. It reads JSONL commands on stdin
// and emits scripted NDJSON events on stdout. Behavior is selected by the
// FAKEPI_MODE env var so the same binary serves every scenario.
//
// Modes:
//
...
func main() {
	...
	case "badlines":
		runBadLines()
	default:
		fmt.Fprintf(os.Stderr, "fakepi: unknown FAKEPI_MODE %q\n", mode)
		os.Exit(2)
	}
}
