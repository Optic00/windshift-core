...
func setupApprovalChannelFixture(t *testing.T) approvalChannelFixture {
	...
	insertRequest := func(channelID, itemNumber int) int {
		...
		var itemID int
		if err := db.QueryRow(`
			INSERT INTO items (workspace_id, workspace_item_number, title, status_id, channel_id, creator_id, creator_portal_customer_id)
			VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id
		`, workspaceID, itemNumber, fmt.Sprintf("Channel %d approval", channelID), statusID, channelID, userID, customerID).Scan(&itemID); err != nil {
