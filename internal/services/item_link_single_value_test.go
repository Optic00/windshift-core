...
func TestReplaceSingleValueFieldLinkRollsBackWhenCreateFails(t *testing.T) {
	...
	sourceID := insertItemLinkBatchRow(t, db,
		`INSERT INTO items (workspace_id, workspace_item_number, title) VALUES (?, 1, 'Source')`, workspaceID,
	)
	oldTargetID := insertItemLinkBatchRow(t, db,
		`INSERT INTO items (workspace_id, workspace_item_number, title) VALUES (?, 2, 'Old target')`, workspaceID,
	)
	newTargetID := insertItemLinkBatchRow(t, db,
		`INSERT INTO items (workspace_id, workspace_item_number, title) VALUES (?, 3, 'New target')`, workspaceID,
	)
