...
func newKnowledgeTestDB(t *testing.T) (database.Database, *PermissionService) {
	...
	if _, err := db.Exec(`INSERT INTO users (id, email, username, first_name, last_name, password_hash, is_active) VALUES (1, 'e@a', 'alice', 'A', 'A', 'h', 1), (2, 'b@b', 'bob', 'B', 'B', 'h', 1)`); err != nil {
	...
	if _, err := db.Exec(`INSERT INTO users (id, email, username, first_name, last_name, password_hash, is_active) VALUES (998, 'p@p', 'phantom', 'P', 'P', 'h', 1)`); err != nil {
...
func TestKnowledgeRetrieval_FiltersByPagePermission(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO users (id, email, username, first_name, last_name, password_hash, is_active) VALUES (3, 'c@c', 'carol', 'C', 'C', 'h', 1)`); err != nil {
