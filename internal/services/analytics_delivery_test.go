...
func (f analyticsDeliveryFixture) insertItem(
	...
	t.Helper()
	result, err := f.db.ExecWrite(`
		INSERT INTO items (
			workspace_id, workspace_item_number, title, description, status_id,
			created_at, updated_at, last_active_at
		)
		VALUES (?, ?, ?, '', ?, ?, ?, ?)
	`, f.workspaceID, number, title, statusID, createdAt, createdAt, createdAt)
	if err != nil {
