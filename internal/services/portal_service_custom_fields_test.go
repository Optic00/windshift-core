...
func TestGetCustomFieldsForChannelOnlyReturnsFieldsBoundToCreateScreen(t *testing.T) {
	...
	for label, statement := range map[string]string{
		...
		"screen field":            `INSERT INTO screen_fields (screen_id, field_type, field_identifier) VALUES (?, 'custom', ?)`,
	} {
		var args []interface{}
		switch label {
		...
		case "screen field":
			args = []interface{}{screenID, strconv.Itoa(allowedFieldID)}
		}
	...
	for _, fieldID := range []int{allowedFieldID, hiddenFieldID} {
		if _, err := db.ExecWrite(`
			...
			VALUES (?, ?, 'custom', false, ?)
		`, requestTypeID, strconv.Itoa(fieldID), fieldID); err != nil {
