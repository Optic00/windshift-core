...
func TestSyncReleaseBranches_EmitsEvent_Once(t *testing.T) {
	...
	if err := svc.syncReleaseBranches(context.Background(), prov, "owner", "repo", 1, 99); err != nil {
...
func TestSyncTags_EmitsEvent_Once(t *testing.T) {
	...
	if err := svc.syncTagsAndReleases(context.Background(), prov, "owner", "repo", 1, 99); err != nil {
		t.Fatalf("first run: %v", err)
	}
	if got := len(emitter.snapshot()); got != 1 {
		t.Fatalf("first run emitted %d events, want 1", got)
...
func TestSyncTags_ProviderError_NoMark(t *testing.T) {
	...
	err := svc.syncTagsAndReleases(context.Background(), ep, "owner", "repo", 1, 99)
	if err == nil {
