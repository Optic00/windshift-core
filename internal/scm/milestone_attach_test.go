...
// scm_processed_commits for idempotency.
func newMilestoneAttachTestDB(t *testing.T) database.Database {
	...
	stmts := []string{
		...
		`CREATE TABLE scm_processed_commits (
...
// the rows, and scm_processed_commits is populated for idempotency.
func TestMilestoneAttacher_AttachesIssuesFromCommits(t *testing.T) {
	...
	for _, c := range commits {
		...
		_ = att.markCommitProcessed(context.Background(), 7, c.SHA)
	}
	...
	var n int
	if err := db.QueryRow(`SELECT COUNT(*) FROM scm_processed_commits WHERE workspace_repository_id = 7`).Scan(&n); err != nil {
...
// dedupe: pre-seeding scm_processed_commits causes
...
func TestMilestoneAttacher_IsAlreadyProcessedSkips(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO scm_processed_commits(commit_sha, workspace_repository_id) VALUES ('c1', 7)`); err != nil {
		t.Fatalf("seed: %v", err)
	}
	done, err := att.commitAlreadyProcessed(context.Background(), 7, "c1")
	if err != nil {
