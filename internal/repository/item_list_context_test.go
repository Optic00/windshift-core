...
func TestFindAllWithDetailsUsesLatestCurrentStatusTransition(t *testing.T) {
	...
	insertItem := func(number, statusID int, title string) int {
		...
		result, err := db.ExecWrite(`
			INSERT INTO items (workspace_id, workspace_item_number, title, description, status_id, created_at, updated_at)
			VALUES (?, ?, ?, '', ?, ?, ?)
		`, workspaceID, number, title, statusID, createdAt.Format(time.RFC3339Nano), createdAt.Format(time.RFC3339Nano))
		if err != nil {
...
func BenchmarkCurrentStatusTransitionLookup(b *testing.B) {
	...
	itemResult, err := db.ExecWrite(`
		INSERT INTO items (workspace_id, workspace_item_number, title, description, status_id)
