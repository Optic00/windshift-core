...
func (c *scriptedClient) ChatCompletion(_ context.Context, req ChatCompletionRequest) (*ChatCompletionResponse, error) {
	c.requests = append(c.requests, req)
	if len(c.responses) == 0 {
		return &ChatCompletionResponse{Choices: []Choice{{Message: Message{Role: "assistant", Content: "done"}, FinishReason: "stop"}}}, nil
	}
	resp := c.responses[0]
...
func TestRunAgentTerminalToolForcesFinalNoToolsAndSuppressesDuplicate(t *testing.T) {
	client := &scriptedClient{responses: []*ChatCompletionResponse{
		{
			Choices: []Choice{{
