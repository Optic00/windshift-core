...
describe('item activity attribution request graph', () => {
  it('loads attributed comments without per-agent owner requests', async () => {
    const apiClient = {
      getComments: vi.fn().mockResolvedValue([{ id: 1, agent_owner_name: 'Agent Owner' }]),
      getAgentOwner: vi.fn(),
    };

    const comments = await loadAttributedComments(apiClient, 42);

    expect(apiClient.getComments).toHaveBeenCalledWith(42);
    expect(apiClient.getAgentOwner).not.toHaveBeenCalled();
    expect(agentOwnerName(comments[0])).toBe('Agent Owner');
  });
