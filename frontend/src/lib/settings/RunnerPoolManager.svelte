    ...
    <section class="space-y-3">
      ...
      {:else}
        <DataTable columns={tokenColumns} data={tokens} keyField="id" emptyMessage="No tokens yet">
          ...
          {#snippet actions(tok)}
            {#if !tok.revoked_at}
              <button class="p-1.5 rounded hover:opacity-80" style="color: var(--ds-text-danger);" title="Revoke token" onclick={() => revokeToken(tok)}>
                <Trash2 size={14} />
              </button>
            {/if}
          {/snippet}
      ...
      {:else}
        <DataTable columns={instanceColumns} data={instances} keyField="id" emptyMessage="No runners registered">
          ...
          {#snippet actions(inst)}
            {#if !inst.revoked_at}
              <button class="p-1.5 rounded hover:opacity-80" style="color: var(--ds-text-danger);" title="Revoke runner" onclick={() => revokeInstance(inst)}>
                <Trash2 size={14} />
              </button>
            {/if}
          {/snippet}
